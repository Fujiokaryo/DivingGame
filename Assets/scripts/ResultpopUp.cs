...
public class ResultpopUp : MonoBehaviour
{
    ...
    void Start()
    {
        //各アルファ値を０にして、リザルト表示を非表示にしておく
        canvasGroupPopUp.alpha = 0;
        canvasGrouptxt.alpha = 0;

        //ボタンのonclickx1イベントにメソッドを登録
 ...
 /// </summary>
    ...
    {
        //CanvasGroupのアルファ値を変更してリザルト表示。onCompleteメソッドを利用すると、
        //DOFadeメソッドの処理を終わった後に登録してある処理を自動的に実行してくれる
        ...
        //タイトルの大きさを変更するため、いったん、現在の大きさを変数に代入して保持する
        Vector3 scale = imgTitle.transform.localScale;

        //タイトルの大きさを０にして非表示状態にする
        ...
        //タイトルの大きさを0.25秒かけて1.5倍にする
        sequence.Append(imgTitle.transform.DOScale(scale, 0.15f));

        //前のappendメソッドが終了してから、このメソッドが処理される
        //タイトルの大きさを0.15秒かけて元の大きさに戻す　=>　一瞬大きくなる演出になる
        sequence.Append(imgTitle.transform.DOScale(scale, 0.15f));
    ...
    private void OnClickRetry()
    {
        ...
        //タップ済にする　以降は上にあるif分の制御により、タップしてもこちらの処理にはこなくなる
        isClickable = true;
